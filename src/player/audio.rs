...
use super::state::{PlaybackStatus, PlayerCommand, PlayerState};
...
impl AudioOutput {
    ...
    pub fn new(
        ...
        let stream = match supported_config.sample_format() {
            SampleFormat::F32 => build_stream::<f32>(&device, &config, audio_rx, callback_state),
            SampleFormat::I16 => build_stream_i16(&device, &config, audio_rx, callback_state),
            format => {
...
impl AudioThreadContext {
    ...
    fn handle_command(&mut self, cmd: PlayerCommand, state: &RwLock<PlayerState>) -> bool {
        match cmd {
