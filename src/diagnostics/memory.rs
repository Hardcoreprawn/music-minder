    ...
    pub fn to_checks(&self) -> Vec<DiagnosticCheck> {
        ...
        let (status, recommendation) = if total_gb >= 8.0 {
            ...
            (CheckStatus::Warning, Some(
                "4-8GB RAM is adequate but may be limiting for large projects.".to_string()
            ))
        } else {
            (CheckStatus::Fail, Some(
                "Less than 4GB RAM. System will page frequently, causing audio glitches.".to_string()
            ))
        };
        ...
        let (status, recommendation) = if available_gb >= 2.0 {
            ...
            (CheckStatus::Warning, Some(
                "Available RAM is getting low. Close unused applications.".to_string()
            ))
        } else {
            (CheckStatus::Fail, Some(
        ...
        checks.push(DiagnosticCheck {
            ...
            category: "Memory".to_string(),
            status,
            value: format!("{:.1} GB ({:.1}% free)", available_gb, 
        ...
        let (status, recommendation) = if self.memory_load < 70 {
            ...
            (CheckStatus::Warning, Some(
                "Memory pressure is elevated. Monitor for paging activity.".to_string()
            ))
        } else {
            (CheckStatus::Fail, Some(
                "High memory pressure! System is likely swapping to disk.".to_string()
            ))
        };
...
mod tests {
    ...
    fn test_query_memory_info() {
        ...
        println!("RAM: {:.1} GB total, {:.1} GB available ({}% used)",
            info.total_gb(), info.available_gb(), info.memory_load);
    }
}
