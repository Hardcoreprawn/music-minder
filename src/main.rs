...
fn main() -> anyhow::Result<()> {
    ...
    // Initialize the async runtime for CLI commands
    ...
    match &cli.command {
        ...
        Some(Commands::Identify { path, api_key, write, fill_only }) => {
            rt.block_on(async {
                ...
                let config = enrichment::EnrichmentConfig {
                    ...
                    min_confidence: 0.5, // Lower threshold for CLI to show more results
        ...
        Some(Commands::CheckTools) => {
            ...
            if let Some(version) = enrichment::fingerprint::get_fpcalc_version() {
                ...
                println!("  Install Chromaprint:");
                println!("    Windows: winget install AcoustID.Chromaprint");
                println!("    macOS:   brew install chromaprint");
                println!("    Linux:   apt install libchromaprint-tools");
            }
        ...
        Some(Commands::Enrich { path, api_key, write, fill_only, recursive, min_confidence, dry_run, db }) => {
            ...
            rt.block_on(async {
                ...
                let pool = if let Some(db_path) = db {
        ...
        Some(Commands::Check { path, db, errors_only, verbose }) => {
            rt.block_on(async {
                ...
                let records = if *errors_only {
                    ...
                    // Get all records or filter by path
                    let all_statuses = [
                ...
                if records.is_empty() {
                    ...
                    if path.is_some() {
                        println!("Try running 'enrich --db {}' first to scan files.", db.display());
                    }
                } else {
                    // Print summary
