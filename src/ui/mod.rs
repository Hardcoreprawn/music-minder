...
impl MusicMinder {
    pub fn new() -> (Self, Task<Message>) {
        let init_db = Task::perform(
            async { crate::db::init_db("sqlite:music_minder.db").await.map_err(|e| e.to_string()) },
    ...
    pub fn subscription(&self) -> Subscription<Message> {
        ...
        if s.preview_loading {
            subscriptions.push(Subscription::run_with_id(
                "preview-organize",
                streams::preview_stream(s.pool.clone(), s.organize_pattern.clone(), s.organize_destination.clone()),
            ));
        }
        ...
        if s.player_state.status == PlaybackStatus::Playing {
            ...
            subscriptions.push(
                time::every(Duration::from_millis(33))
                    .map(|_| Message::PlayerVisualizationTick)
            );
        }
    ...
    pub fn view(&self) -> Element<'_, Message> {
        let content: Element<Message> = match &self.state {
            ...
            AppState::Error(e) => text(format!("Error: {}", e)).size(30).color([0.9, 0.0, 0.0]).into(),
        };
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        ...
        match &message {
            Message::DbInitialized(result) => return update::handle_db_init(&mut self.state, result.clone()),
        ...
        let AppState::Loaded(s) = &mut self.state else { return Task::none() };

        match &message {
            // Navigation
            Message::SwitchPane(pane) => { s.active_pane = *pane; }
            
            // Scroll updates
            Message::ScrollChanged(v) => { s.scroll_offset = v.absolute_offset().y; s.viewport_height = v.bounds().height; }
            Message::PreviewScrollChanged(v) => { s.preview_scroll_offset = v.absolute_offset().y; s.preview_viewport_height = v.bounds().height; }
            
            // Path updates
            Message::PathChanged(p) => { s.scan_path = PathBuf::from(p); }
            Message::PathPicked(Some(p)) => { s.scan_path = p.clone(); }
            ...
            // Scan messages
            Message::ScanPressed | Message::ScanStopped | Message::ScanFinished | Message::ScanEventReceived(_) => {
                return update::handle_scan(s, &message);
            }

            // Organize messages  
            Message::OrganizeDestinationChanged(_) | Message::OrganizePatternChanged(_) |
            Message::PickOrganizeDestination | Message::OrganizeDestinationPicked(_) |
            Message::OrganizePreviewPressed | Message::OrganizePreviewBatch(_) |
            Message::OrganizePreviewComplete | Message::OrganizeCancelPressed |
            Message::OrganizeConfirmPressed | Message::OrganizeFileComplete(_) |
            ...
            // Enrichment messages
            Message::EnrichmentApiKeyChanged(_) | Message::EnrichmentTrackSelected(_) |
            Message::EnrichmentIdentifyPressed | Message::EnrichmentIdentifyResult(_) |
            Message::EnrichmentClearResult | Message::EnrichmentWriteTagsPressed |
            Message::EnrichmentWriteTagsResult(_) => {
                return update::handle_enrichment(s, message);
            }
            
            // Player messages
            Message::PlayerPlay | Message::PlayerPause | Message::PlayerToggle |
            Message::PlayerStop | Message::PlayerNext | Message::PlayerPrevious |
            Message::PlayerSeek(_) | Message::PlayerVolumeChanged(_) |
            Message::PlayerPlayTrack(_) | Message::PlayerQueueTrack(_) |
