...
mod e2e_tests {
    ...
    fn test_path_operations() {
        ...
        assert_eq!(file1.parent().unwrap().file_name().unwrap().to_str().unwrap(), "Album1");
    }
}
...
mod enrichment_e2e_tests {
    use super::super::test_mocks::{MockAcoustIdServer, MockMusicBrainzServer};
    ...
    async fn test_acoustid_mock_server() {
        ...
        let url = format!("{}/v2/lookup?client=test&duration=180&fingerprint=test", mock.base_url());
    ...
    async fn test_musicbrainz_mock_server() {
        ...
        let client = reqwest::Client::new();
        let url = format!("{}/ws/2/recording/4f4868cb-dcc2-4998-bafe-a97bdda95e7f", mock.base_url());
